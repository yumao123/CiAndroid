...
def add_enterprise():
	...
	if request.method == 'POST':
		...
		ret = EnterpriseInfoModule.initialize_enterprise_info(enterprise_number)
...
def check_client_name(update_url):
	...
	try:
		...
		raise Exception("Update url is not legal, " + update_url)
...
def get_html():
	...
	if request.method == 'POST':
		...
		try:
			...
			return make_response(jsonify({"result": str(e)}), 400) 
		# ssh下载文件
